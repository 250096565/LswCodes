...
namespace WxPayAPI
{
    public class NativePay
    {
        ...
        * @return 模式二URL
        */
        public string GetPayUrl(string productId, out string outtradeno)
        {
            Log.Info(this.GetType().ToString(), "Native pay mode 2 url is producing...");
            outtradeno = WxPayApi.GenerateOutTradeNo();
            WxPayData data = new WxPayData();
            data.SetValue("body", "用于测试微信支付");//商品描述
            data.SetValue("attach", "走起来");//附加数据
            data.SetValue("out_trade_no", outtradeno);//随机字符串
            data.SetValue("total_fee", 1);//总金额
            data.SetValue("time_start", DateTime.Now.ToString("yyyyMMddHHmmss"));//交易起始时间
            data.SetValue("time_expire", DateTime.Now.AddMinutes(10).ToString("yyyyMMddHHmmss"));//交易结束时间
            data.SetValue("goods_tag", "good");//商品标记
